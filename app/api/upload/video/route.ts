...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    // Validate MIME type
    const fileValidation = validateFile(file.size, file.type, "video");
    ...
    const contentValidation = validateFileContent(buffer, file.type, "video");
    ...
    const key = generateKey(userId, `${timestamp}.${ext}`, "video");
