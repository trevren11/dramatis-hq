...
export const resumeConfigurations = pgTable(
  "resume_configurations",
  {
    ...
    template: varchar("template", { length: 50 }).default("theatrical").notNull(),
    ...
    includeHeight: boolean("include_height").default(true),
    includeHair: boolean("include_hair").default(true),
    includeEyes: boolean("include_eyes").default(true),
    customHeaderText: text("custom_header_text"),
    customSkillsText: text("custom_skills_text"),
    customTrainingText: text("custom_training_text"),
    createdAt: timestamp("created_at", { mode: "date" }).defaultNow().notNull(),
    updatedAt: timestamp("updated_at", { mode: "date" }).defaultNow().notNull(),
  },
